...
for subject_index in range(0, 8):
    ...
    plot_save_graph(filename=f"confident_intervals_subject_{subject_index + 3}.png", fig = fig)
...
for subject_index in range(0, 8):
    ...
    plot_save_graph(filename=f"statistically_significant_subject_{subject_index + 3}.png", fig = fig)
...
eeg_time, eeg_data, rowcol_id, is_target = load_p300_data.load_training_eeg(subject=8)
event_sample, is_target_event = plot_p300_erps.get_events(rowcol_id, is_target)
...
# get N2 and p3b epoch erps and time ranges  vcv
...
cbar_fig = colorbar.ax.figure
cbar_fig.savefig(f"output/topo_target_P3b")

im, colorbar = plot_topo(channel_names, nontarget_erp_n2.T, title="N2")
cbar_fig = colorbar.ax.figure
cbar_fig.savefig(f"output/topo_nontarget_N2.png")
